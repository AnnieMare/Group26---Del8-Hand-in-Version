...
@Component({
  selector: 'app-set-nmo-goals',
  templateUrl: './set-nmo-goals.component.html',
  styleUrls: ['./set-nmo-goals.component.scss']
})
export class SetNMOGoalsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.GoalsService.getNMOOverseer().subscribe(data=>{
  ...
  addNewMemberOrientationGoalForm(form) {
    ...
    this.GoalsService.setNMOGoal(data).subscribe(x =>{
      if(x == "New Member Oriantation Monthly goal set successfully.")
      {
        this.Message = x;
        this.displayMessage('top-right', 'success');
        this.NewMemberOrientation.reset();
      }
      else
      {
        this.Message = x;
