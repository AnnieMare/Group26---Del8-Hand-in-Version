...
@Component({
  selector: 'app-set-homecell-atten-goals',
  templateUrl: './set-homecell-atten-goals.component.html',
  styleUrls: ['./set-homecell-atten-goals.component.scss']
})
export class SetHomecellAttenGoalsComponent implements OnInit {
  ...
  constructor(private router: Router, private GoalsService: GoalsServiceService,private formBuilder: FormBuilder, private sidebarService: NbSidebarService,
    ...
    FirstTimeVisitors: ['', Validators.required],
    Salvations: ['', Validators.required],
    Overseer: ''});
  ...
  addHomecellAttGoalForm(form) {
    ...
    this.GoalsService.setHomecellAttGoal(data).subscribe(x =>{
      if(x == "Weekly Homecell goal have been set successfully.")
      {
        this.Message = x;
