...
@Component({
  selector: 'app-set-structure-growth-goals',
  templateUrl: './set-structure-growth-goals.component.html',
  styleUrls: ['./set-structure-growth-goals.component.scss']
})
export class SetStructureGrowthGoalsComponent implements OnInit {
  StructureGrowth: any;
   Person:any;
  session:any;
  Message: Object;
  ...
  ngOnInit(): void {
    ...
    this.GoalsService.getStructureOverseer().subscribe(data=>{
  ...
  addStructureGrowthGoalForm(form) {
    ...
    this.GoalsService.setStructureGrowthGoal(data).subscribe(x =>{
      if(x == "Structure Growth Goal was set successfully.")
      {
        this.Message = x;
        this.displayMessage('top-right', 'success');
        this.StructureGrowth.reset();
      }
      else
      {
        this.Message = x;
