...
@Component({
  selector: 'app-set-zone-growth-goals',
  templateUrl: './set-zone-growth-goals.component.html',
  styleUrls: ['./set-zone-growth-goals.component.scss']
})
export class SetZoneGrowthGoalsComponent implements OnInit {
  ZoneGrowth: any;
  Person:any;
  session:any;
  Message: Object;
  ...
  addZoneGrowthGoalForm(form) {
    ...
    this.GoalsService.setZoneGrowthGoal(data).subscribe(x =>{
      if(x == "Zone Growth Goal was set successfully.")
      {
        this.Message = x;
        this.displayMessage('top-right', 'success');
        this.ZoneGrowth.reset();
      }
      else
      {
        this.Message = x;
